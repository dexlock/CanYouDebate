...
label:"",selected:!s}):s||a[""].unshift({id:"?",label:"",selected:!0}));y=0;for(A=c.length;y<A;y++){d=c[y];k=a[d];v.length<=y?(u={element:D.clone().attr("label",d),label:k.label},x=[u],v.push(x),f.append(u.element)):(x=v[y],u=x[0],u.label!=d&&u.element.attr("label",u.label=d));E=null;I=0;for(F=k.length;I<F;I++)s=k[I],(d=x[I+1])?(E=d.element,d.label!==s.label&&E.text(d.label=s.label),d.id!==s.id&&E.val(d.id=s.id),d.element.selected!==s.selected&&(d.selected!==s.selected&&E.prop("selected",d.selected=s.selected))):(""===s.id&&w?J=
